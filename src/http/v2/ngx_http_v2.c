...
ngx_http_v2_init(ngx_event_t *rev)
{
    ...
    h2c->priority_limit = 100;
    h2c->concurrent_streams_limit = 16;
...
ngx_int_t
ngx_http_v2_adjust_streams(ngx_http_request_t *req, ngx_int_t concurrent_streams) {
    ngx_http_v2_connection_t *h2c;
    size_t                    len;
    ngx_buf_t                *buf;
    ngx_chain_t              *cl;
    ...
    h2c = req->stream->connection;

    
    if(concurrent_streams <= 0) {
        h2scf = ngx_http_get_module_srv_conf(h2c->http_connection->conf_ctx,
                                         ngx_http_v2_module);
        concurrent_streams += h2scf->concurrent_streams;
        if(concurrent_streams < 4) concurrent_streams = 4;
    }

    if(concurrent_streams == h2c->concurrent_streams_limit) {
        return NGX_OK;
    }
    ...
    len = NGX_HTTP_V2_SETTINGS_PARAM_SIZE * 1;
    ...
    *buf->last++ = NGX_HTTP_V2_NO_FLAG;
    ...
    h2c->concurrent_streams_limit = (ngx_uint_t)concurrent_streams;
...
ngx_http_v2_parse_method(ngx_http_request_t *r, ngx_str_t *value)
{
    ...
    r->orig_method_name.len = r->method_name.len = value->len;
    r->orig_method_name.data = r->method_name.data = value->data;
