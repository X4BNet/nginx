...
static int xenc_envelope_open(EVP_PKEY *priv_key, unsigned char *ciphertext, int ciphertext_len,
...
static EVP_PKEY* xenc_open_private_key(const char *filename){
...
static unsigned char* xenc_decrypt_key(ngx_pool_t *pool, EVP_PKEY *priv_key, const char* fname, int* encLen, char** err){
    ...
    *encLen = -1;
    ...
    plainTextLen = xenc_envelope_open(priv_key, encData, encSize, keyData, keySize, ivData, plaintextData);
    ...
    ngx_str_t *key, ngx_array_t *passwords)
{
    ...
    if(cnf != NULL && ngx_strncmp(key->data, "e:", sizeof("e:") - 1) == 0){
        ...
        key_buffer = xenc_decrypt_key(pool, ecf->pkey, (const char*)key->data + (sizeof("e:") - 1), &encLen, err);
        if(key_buffer == NULL){
...
ngx_openssl_pkey(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    oscf->pkey = xenc_open_private_key((char *) value[1].data);
    if(!oscf->pkey){
        return "is not valid";
    }
